...
router.post('/', requireAuth, uploadPdf.single('resume'), async (req, res) => {
  try {
    ...
    const { error: upErr } = await supabaseService.storage.from('resumes').upload(objectName, req.file.buffer, {
    ...
    const { data, error } = await supabaseService.from('applications').insert(insertPayload).select().single();
...
router.get('/:id/resume-url', requireAuth, async (req, res) => {
  ...
  if (appRec.user_id !== req.user.id && req.user.role !== 'admin') return res.status(403).json({ error: 'Forbidden' });

  const { data, error } = await supabaseService.storage.from('resumes').createSignedUrl(appRec.resume_path, 600);
  if (error) return res.status(500).json({ error: error.message });
...
router.delete('/:id/resume', requireAuth, async (req, res) => {
  ...
  const { error } = await supabaseService.storage.from('resumes').remove([appRec.resume_path]);
  if (error) return res.status(500).json({ error: error.message });

  await supabaseService.from('applications').update({ resume_path: null, updated_at: new Date().toISOString() }).eq('id', appRec.id);
  res.json({ ok: true });
});
